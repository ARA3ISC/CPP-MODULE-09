...
int main(int ac, char **av)
{
	...
	try
	{
		...
		if (ac == 2)
			throw std::invalid_argument("There is just one number");
		...
		std::cout << std::fixed << std::setprecision(6) << "Time to process a range of " << v.size() << " with std::vector<int> : " << vector_f_time << " us" << std::endl;
		std::cout << std::fixed << std::setprecision(6) << "Time to process a range of " << d.size() << " with std::deque<int>  : " << deque_f_time << " us" << std::endl;
	}
